...
const petType = () => {
    ...
    for (let l = 0; l < pets.length; l++){
        ...
        domString += `<h2>${pets[l].name}</h2>`;  
        domString += `<img src=${pets[l].imageUrl}></img>`;  
        domString += `<p>${pets[l].color}</p>`;
        domString += `<p>${pets[l].specialSkill}</p>`;
        domString += `<p>${pets[l].type}</p>`;
        domString += `</div>`;
    };

    printToDom('buyAPet', domString);
};
